...
func TestDatabaseQueryBadConnection(t *testing.T) {
	db := NewContext("localhost", 5432, "test", "wrongpassword", "applicationmanager_loginservice")
...
func TestDatabaseDeleteAccount(t *testing.T) {
	...
	configPath := fmt.Sprintf("%s/.secret/application_manager_login_service_db.yml", homeDir)
	db, err := NewContextFromConfig(configPath)
...
func TestDatabaseGetAccountByUsernameBadConnection(t *testing.T) {
	db := NewContext("localhost", 5432, "username", "wrongpassword", "")
